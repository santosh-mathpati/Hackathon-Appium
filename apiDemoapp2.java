...
public class apiDemoapp2 {
    ...
    public void LaunchApk1() throws MalformedURLException, URISyntaxException, InterruptedException {
        ...
        WebElement animationElement = wait.until(ExpectedConditions.elementToBeClickable(
        ...
        // Wait for "View Flip" element and click
        WebElement viewFlipElement = wait.until(ExpectedConditions.elementToBeClickable(
                AppiumBy.xpath("//android.widget.TextView[@content-desc='View Flip']")));
        viewFlipElement.click();
        System.out.println("Clicked on View Flip button");

        // Wait for the "Flip" button
        WebElement flipElement = wait.until(ExpectedConditions.elementToBeClickable(
                AppiumBy.accessibilityId("Flip")));

        // Tap on Flip using coordinates
        tapOnElement(driver, flipElement);
        System.out.println("Tapped on Flip button");

        Thread.sleep(3000);
        driver.quit();
    }

    // Function to tap on an element using coordinates
    private void tapOnElement(AndroidDriver driver, WebElement element) {
        Point location = element.getLocation();
